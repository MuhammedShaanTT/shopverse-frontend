                                        ...
                                        {editingId === p.id ? (
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                                                <input value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })}
                                                    style={{ padding: '0.5rem 0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontSize: '0.85rem', background: 'var(--bg-input)', color: 'var(--text-primary)', fontFamily: 'var(--font-sans)' }} />
                                                <div style={{ display: 'flex', gap: '0.4rem' }}>
                                                    <input type="number" step="0.01" value={editForm.price}
                                                        onChange={e => setEditForm({ ...editForm, price: e.target.value })}
                                                        style={{ padding: '0.5rem 0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontSize: '0.85rem', flex: 1, background: 'var(--bg-input)', color: 'var(--text-primary)', fontFamily: 'var(--font-sans)' }}
                                                        placeholder="Price" />
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                <div>
                                                    ...
                                                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>₹{p.price} · Stock: {p.stock} · {p.categoryName}</p>
                                                </div>
                                                <div style={{ display: 'flex', gap: '0.3rem' }}>
                                                    <button className="btn-small" onClick={() => startEdit(p)} style={{ padding: '0.3rem 0.6rem' }}>
                                                        <FiEdit />
                                                    </button>
